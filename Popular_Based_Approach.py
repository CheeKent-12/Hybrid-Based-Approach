...
num_recommendations = st.sidebar.number_input("Enter the number of books to recommend", min_value=1, value=5)
quantile_value = st.sidebar.slider("Select the quantile for minimum rating count (0 - 1)", 0.0, 1.0, 0.75, 0.01)

# Extract relevant features and remove null values
books_data = df[['authors', 'original_publication_year', 'title', 'average_rating', 'ratings_count', 'image_url']].copy()
books_data.dropna(inplace=True)

# Change data type of original_publication_year to int from float
books_data['original_publication_year'] = books_data['original_publication_year'].astype(int)
...
for i, (author, year, title, image_url) in enumerate(recommended_books[['authors', 'original_publication_year', 'title', 'image_url']].values, start=1):
